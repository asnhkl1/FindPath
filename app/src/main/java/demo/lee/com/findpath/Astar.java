...
public class Astar {
    public final static int DIRECT_VALE = 10;
    public final static int OBLIQUE_VALE = 14;
    ...
    private boolean canAddNodeToOpen(MapInfo mapInfo, int x, int y) {
        //是否在地图中
        if (x < 0 || x >= mapInfo.map_width || y < 0 || y >= mapInfo.map_height) return false;
        //判断是否能过
    ...
    private void addNeighborNodeInOpen(MapInfo mapInfo, Node current) {
        ...
        addNeighborNodeInOpen(mapInfo, current, x - 1, y, DIRECT_VALE);
        addNeighborNodeInOpen(mapInfo, current, x, y - 1, DIRECT_VALE);
        addNeighborNodeInOpen(mapInfo, current, x + 1, y, DIRECT_VALE);
        addNeighborNodeInOpen(mapInfo, current, x, y + 1, DIRECT_VALE);

        addNeighborNodeInOpen(mapInfo, current, x + 1, y + 1, OBLIQUE_VALE);
        addNeighborNodeInOpen(mapInfo, current, x - 1, y - 1, OBLIQUE_VALE);
        addNeighborNodeInOpen(mapInfo, current, x + 1, y - 1, OBLIQUE_VALE);
        addNeighborNodeInOpen(mapInfo, current, x - 1, y + 1, OBLIQUE_VALE);
    }
